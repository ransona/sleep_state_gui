...
def run_sleep_scoring(user_id, exp_id):
    ...
    animal_id, remote_root, processed_root, exp_dir_processed, exp_dir_raw = \
    ...
    noverlap = win_len_samples - step_len_samples  # 3 s overlap
    ...
    if make_qc_figs:
        ...
        # emg spectrogram (coarse, just for QC)
    ...
    # -----------------------------------------------------------------
    # emg gmm (for thresholding movement / emg amplitude) + scoring
    # -----------------------------------------------------------------
    ...
    sleep_state = {
        ...
        'wheel_speed_threshold': 1.0,                         # abs(wheel) > this
