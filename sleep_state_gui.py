...
class VideoAnalysisApp(QMainWindow):
    ...
    def loadData(self):
        ...
        if os.path.isfile(sleep_state_path):
            ...
            if reply == QMessageBox.Yes:
                try:
                    # use animalID and expID as requested
                    sleep_state = run_sleep_scoring(self.animalID, self.expID)
                except Exception as e:
    ...
    def runSleepScoringClicked(self):
        ...
        try:
            animalID, _, _, _, _ = organise_paths.find_paths(user_id, exp_id)
        except Exception as e:
            QMessageBox.critical(self, "Path Error", f"Error resolving paths:\n{e}")
            return

        try:
            run_sleep_scoring(animalID, exp_id)
        except Exception as e:
            QMessageBox.critical(self, "Sleep scoring error", f"Error running sleep scoring:\n{e}")
            return
    ...
    def plotTraces(self):
        ...
        ratio = delta_masked / theta_masked
...
if __name__ == "__main__":
    ...
    # start maximized to fill the screen; plots resize with the window
    win.showMaximized()
    sys.exit(app.exec_())
