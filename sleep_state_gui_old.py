...
from sleep_score_old import run_sleep_scoring
...
class VideoAnalysisApp(QMainWindow):
    ...
    def loadData(self):
        ...
        if os.path.isfile(sleep_state_path):
            ...
            if reply == QMessageBox.Yes:
                try:
                    # now pass userID and expID (not animalID)
    ...
    def runSleepScoringClicked(self):
        ...
        # we still resolve paths (for side-effects / consistency), but pass userID to scoring
        try:
        ...
        try:
            # now pass user_id and exp_id directly
    ...
    # --- frame <-> time ---
    def _current_time_from_frame(self, frame=None):
        if frame is None:
            frame = self.slider.value()
        if not self.loaded or self.fps is None or self.fps <= 0:
            return 0.0
    ...
    def updateFrame(self):
        ...
        frame_left = self.playVideoFrame(position, self.video_path_left, self.left_eyedat)
        ...
        frame_right = self.playVideoFrame(position, self.video_path_right, self.right_eyedat)
        ...
        if self.vlines:
            t = self._current_time_from_frame(position)
    ...
    def jumpToViewCenter(self):
        ...
        if self.fps is None or self.fps <= 0:
            ...
            frame = int(round(center_time * self.fps))
    ...
    def plotTraces(self):
        ...
        ax1 = axs[0]
        ax1.plot(self.emg_rms_10hz_t, self.emg_rms_10hz, label="EMG RMS")
        ...
        if self.emg_rms_threshold is not None:
            ax1.axhline(float(self.emg_rms_threshold), color='r', linestyle='--', linewidth=1)

        ax1b = ax1.twinx()
        ax1b.plot(self.wheel_10hz_t, self.wheel_10hz, color='tab:orange', label="Wheel")
        ax1b.set_ylabel("Wheel")
        ax1b.spines['top'].set_visible(False)

        # wheel speed threshold
        if self.wheel_speed_threshold is not None:
            ax1b.axhline(float(self.wheel_speed_threshold), color='r', linestyle='--', linewidth=1)
        ...
        # vertical line in all plots
        self.vlines = []
        current_time = self._current_time_from_frame()
        for ax in axs:
